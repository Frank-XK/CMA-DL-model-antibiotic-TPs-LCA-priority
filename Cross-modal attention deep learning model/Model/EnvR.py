...
class Config:
    def __init__(self):
        ...
        # train
        self.lr = 0.00015
        self.best_model_save_path = './best_models-EnvR/'
        self.visualization_path = './visualization_results-EnvR/'
...
class MolecularDataset(Dataset):
    def __init__(self, df, add_edge_feats=config.add_edge_feats,
        ...
        for _, row in df.iterrows():
            ...
            if mol is not None and row['EnvR'] in self.labels:
...
def plot_confusion_matrices(true_labels, pred_labels, model_names,
    ...
    for model_idx, name in enumerate(model_names):
        ...
        plt.title(f'Confusion Matrix - EnvR')
...
def main():
    ...
    labels = [dataset.labels[row['EnvR']] for row in dataset.data]
    ...
    # Excel
    export_metrics_to_excel(all_results, './model_metrics_summary-EnvR.xlsx')
    ...
    unified_best_path = os.path.join(config.best_model_save_path, 'fusion_gin_best_overall-EnvR.pth')
...
if __name__ == "__main__":
    ...
    config.update(
        ...
        erg_length=441,
        num_classes=2,
        best_model_save_path='./best_models-EnvR/',
        visualization_path='./visualization_results-EnvR/',
